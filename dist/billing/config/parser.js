...
export function parseDeveloperCostConfig(options) {
  ...
  const rawAnnualGrossSalary =
    options?.annualGrossSalary ??
    (legacyMonthlySalary === undefined
      ? DEFAULT_ANNUAL_GROSS_SALARY
      : legacyMonthlySalary * MONTHS_PER_YEAR);
  const rawWorkingHoursPerWeek =
    options?.workingHoursPerWeek ??
    options?.hoursPerWeek ??
    DEFAULT_WORKING_HOURS_PER_WEEK;
  const rawWorkingWeeksPerYear =
    options?.workingWeeksPerYear ??
  ...
  const workingHoursPerWeek =
    parsePositiveNumber(rawWorkingHoursPerWeek) ??
    DEFAULT_WORKING_HOURS_PER_WEEK;
  const workingWeeksPerYear =
    parsePositiveNumber(rawWorkingWeeksPerYear) ??
    DEFAULT_WORKING_WEEKS_PER_YEAR;
  const activeWindowMinutes =
    parsePositiveNumber(rawActiveWindowMinutes) ??
    DEFAULT_ACTIVE_WINDOW_MINUTES;
  const refreshIntervalSeconds =
    parsePositiveNumber(rawRefreshIntervalSeconds) ??
    DEFAULT_REFRESH_INTERVAL_SECONDS;
...
export function parseStoredDeveloperCostConfig(value) {
  ...
  const annualGrossSalary =
    candidate.annualGrossSalary ??
    (legacyMonthlySalary === undefined
      ? undefined
      : legacyMonthlySalary * MONTHS_PER_YEAR);
  const workingHoursPerWeek =
    candidate.workingHoursPerWeek ?? candidate.hoursPerWeek;
  const workingWeeksPerYear =
    candidate.workingWeeksPerYear ?? candidate.weeksPerYear;
  if (
