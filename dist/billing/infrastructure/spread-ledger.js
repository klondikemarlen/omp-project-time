...
export class SpreadBillingLedger {
    ...
    constructor(filePath) {
        this.filePath = filePath ?? path.join(homedir(), ".omp", "developer-cost-status", "spread-billing.json");
    }
    ...
    async update(sessionId, state, nowMs, config, updateKind) {
        return this.withLock(async () => {
            ...
            const settledSessions = settleSpreadDeveloperCostStates([...ledger.sessions].map(([id, entry]) => ({
                sessionId: id,
                state: entry.state,
                config: entry.config,
            })), settlementAtMs);
...
function isStoredConfig(value) {
    ...
    return (typeof monthlySalary === "number" &&
