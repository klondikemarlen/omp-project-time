...
export class TimeLogLedger {
    ...
    constructor(filePath) {
        const defaultPath = path.join(homedir(), ".omp", "developer-attention-status", "time-log.json");
    ...
    async readState() {
        ...
        if (state === undefined)
            throw new Error("Time log state is invalid.");
        return state;
    }
...
function createTimeLogEntry(input) {
    ...
    if (project.length === 0)
        throw new Error("Time log project is required.");
