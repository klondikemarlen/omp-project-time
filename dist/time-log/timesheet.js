export function createTimesheetEntries(intervals, descriptions) {
  const tasksBySession = latestDescriptionsBySession(descriptions);
  const entries = new Map();
  for (const interval of intervals) {
    if (interval.sessionId === undefined || interval.timesheet === undefined)
      continue;
    const task =
      tasksBySession.get(interval.sessionId)?.description ??
      "Unlabeled project work";
    const { projectId, projectName, categoryId, categoryLabel } =
      interval.timesheet;
    const key = [
      interval.sessionId,
...
export function timesheetPreview(entries) {
