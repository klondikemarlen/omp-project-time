...
if (
  ...
  runProjectTimeCli(process.argv.slice(2)).catch((error: unknown) => {
    process.stderr.write(`Project Time command error: ${error instanceof Error ? error.message : String(error)}\n`)
    process.exitCode = 1
  })
}
