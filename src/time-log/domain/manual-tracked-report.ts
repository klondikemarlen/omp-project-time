...
export type ManualTrackedDailyEntry = {
...
export function buildManualTrackedDailyReport(
  entries: readonly TimeLogEntry[],
  project?: string,
): ManualTrackedDailyEntry[] {
  const totals = new Map<string, ManualTrackedDailyEntry>()
  ...
  for (const entry of entries) {
    ...
    while (segmentStartAtMs < entry.endAtMs) {
      ...
      const segmentEndAtMs = localDateAt(entry.endAtMs - 1, entry.timeZone) === localDate
        ? entry.endAtMs
        : nextLocalDateChange(segmentStartAtMs, entry.endAtMs, entry.timeZone, localDate)
