...
export function parseRepositoryIdentity(value: unknown): string | undefined {
  ...
  if (
    ...
    || segments.some(
      (segment) => segment === "." || segment === ".." || !/^[a-z0-9._-]+$/.test(segment),
    )
  ) {
    return undefined
  }
...
function isValidRepositoryHost(host: string): boolean {
  return host
    .split(".")
